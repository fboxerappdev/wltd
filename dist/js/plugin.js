...
(function($) {
	...
	$.fn.cssAnimate = function() {
		...
		function animate_elems() {
			...
			$elemspoint.each(function(index){
				$elemspointpos = $(this);
				$elemspointpos.find('.animateblock').each(function(){
					$elm = $(this);
					$elmClass = $elm.attr('data-animate-class');
					$elmDelay = $elm.attr('data-animate-delay');
					if($elm.hasClass('animated')) { return true; } // if already animated skip to the next item
					
					topcoords = $elemspointpos.offset().top; // element's distance from top of page in pixels
